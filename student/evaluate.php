...
$curriculum = $conn->query("SELECT c.id,s.id as sid,f.id as fid,concat(f.firstname,' ',f.lastname) as faculty,s.code,s.subject FROM curr_list c inner join faculty_list f on f.id = c.faculty_id inner join subject_list s on s.id = c.subject_id where academic_id ={$_SESSION['academic']['id']} and class_id = {$_SESSION['login_class_id']} and c.id not in (SELECT restriction_id from evaluation_list where academic_id ={$_SESSION['academic']['id']} and student_id = {$_SESSION['login_id']} ) ");
?>
	...
	<div class="row">
		<div class="col-md-3">
			<div class="list-group">
				...
				?>
				<a class="list-group-item list-group-item-action <?php echo isset($cid) && $cid == $row['id'] ? 'active' : '' ?>" href="./index.php?page=evaluate&cid=<?php echo $row['id'] ?>&sid=<?php echo $row['sid'] ?>&fid=<?php echo $row['fid'] ?>"><?php echo ucwords($row['faculty']).' - ('.$row["code"].') '.$row['subject'] ?></a>
			<?php endwhile; ?>
			</div>
		</div>	
		<div class="col-md-9">
			<div class="card card-outline card-info">
				...
				<div class="card-body">
					...
					<form id="manage-evaluation">
						...
						<input type="hidden" name="restriction_id" value="<?php echo $cid ?>">
					...
					<div class="form-group">
    					...
    					<textarea name="description" id="description" cols="30" rows="4" class="form-control" required><?php echo isset($comment) ? $comment : '' ?></textarea>
					</div>
		<div class="card-tools">
						<button class="btn btn-sm btn-flat btn-primary bg-gradient-primary mx-1" form="manage-evaluation">Submit Evaluation</button>
					</div>
...
<script>
	...
	$('#manage-evaluation').submit(function(e){
		...
		$.ajax({
			...
			method:'POST',
			data:$(this).serialize() + '&comment=' + encodeURIComponent(comment),
