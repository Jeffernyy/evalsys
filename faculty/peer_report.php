			...
			<div class="callout callout-info" id="printable">
				<div>
					...
					<table width="100%">
						<tr>
							<td width="50%">
								<p><b>Peer Group: <span id="subjectField"></span></b></p>
							</td>
							<td width="50%">
						...
						<tr>
							<td width="50%">
								<p><b>Peer Respondent: <span id="tse"></span></b></p>
							</td>
							<td width="50%">
								<p><b>Average: <span id="overallAverage"></span></b></p>
							</td>
						</tr>
					</table>
...
<script>
	...
	function load_class() {
		...
		$.ajax({
				...
				if (resp) {
					...
					if (Object.keys(resp).length <= 0) {
						...
						Object.keys(resp).map(k => {
							$('#class-list').append('<a href="javascript:void(0)" data-json=\'' + JSON.stringify(resp[k]) + '\' data-id="' + resp[k].id + '" class="list-group-item list-group-item-action show-result">' + "Group " + resp[k].gnum)
						})
			...
			complete: function () {
				end_load()
				anchor_func()
				if ('<?php echo isset($_GET['visor_id']) ?>' == 1) {
					$('.show-result[data-id="<?php echo isset($_GET['visor_id']) ? $_GET['visor_id'] : '' ?>"]').trigger('click')
				} else {
	...
	function anchor_func() {
		$('.show-result').click(function () {
			...
			window.history.pushState({}, null, './index.php?page=peer_report&fid=' + data.visor_id);
			load_report(<?php echo $faculty_id ?>, data.visor_id);
			$('#subjectField').text(data.gnum)
	...
	function load_report($faculty_id, $visor_id) {
		if ($('#preloader2').length <= 0)
			start_load()
		$.ajax({
			url: 'ajax.php?action=get_group_report',
			method: "POST",
			data: { faculty_id: $faculty_id, visor_id: $visor_id },
			error: function (err) {
				console.log(err)
				alert_toast("An Error Occured.", "error");
				end_load()
