			...
			<div class="callout callout-info" id="printable">
				<div>
					...
					<table width="100%">
						<tr>
							<td width="50%">
								<p><b>Peer Name: <span id="nameField"></span></b></p>
							</td>
							<td width="50%">
						...
						<tr>
							<!--<td width="50%"><p><b>Evaluator: <span id="evalField"></span></b></p></td>
						<td width="50%"><p><b>Peer Group Number: <span id="subjectField"></span></b></p></td>-->
						</tr>
					</table>
...
<script>
				...
				if (resp) {
					...
					if (Object.keys(resp).length <= 0) {
						...
						Object.keys(resp).map(k => {
							$('#class-list').append('<a href="javascript:void(0)" data-json=\'' + JSON.stringify(resp[k]) + '\' data-id="' + resp[k].id + '" class="list-group-item list-group-item-action show-result">' + resp[k].peer)
						})
	...
	function anchor_func() {
		$('.show-result').click(function () {
			...
			window.history.pushState({}, null, './index.php?page=result&gid=' + data.fid);
			load_report(<?php echo $faculty_id ?>, data.fid);
			$('#subjectField').text(data.group_num)
			$('#nameField').text(data.peer)
			$('.show-result.active').removeClass('active')
			$(this).addClass('active')
		})
	}
	function load_report($faculty_id, $fid) {
		if ($('#preloader2').length <= 0)
			start_load()
		$.ajax({
			url: 'ajax.php?action=get_peer_report',
			method: "POST",
			data: { faculty_id: $faculty_id, fid: $fid },
			error: function (err) {
				console.log(err)
				alert_toast("An Error Occured.", "error");
				end_load()
