			...
			<div class="callout callout-info" id="printable">
				<div>
					...
					<table width="100%">
						...
						<tr>
							<td width="50%">
								<p><b>Evaluator: <span id="subjectField"></span></b></p>
							</td>
						</tr>
					</table>
...
<script>
				...
				if (resp) {
					...
					if (Object.keys(resp).length <= 0) {
						...
						Object.keys(resp).map(k => {
							$('#class-list').append('<a href="javascript:void(0)" data-json=\'' + JSON.stringify(resp[k]) + '\' data-id="' + resp[k].id + '" class="list-group-item list-group-item-action show-result">' + resp[k].visor)
						})
	...
	function anchor_func() {
		$('.show-result').click(function () {
			...
			window.history.pushState({}, null, './index.php?page=report&fid=' + data.visor_id);
			load_report(<?php echo $faculty_id ?>, data.visor_id);
			$('#subjectField').text(data.visor)
	...
	function load_report($faculty_id, $visor_id) {
		if ($('#preloader2').length <= 0)
			start_load()
		$.ajax({
			url: 'ajax.php?action=get_visor_report',
			method: "POST",
