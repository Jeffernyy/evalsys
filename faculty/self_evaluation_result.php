	...
	<div class="row">
		...
		<div class="col-md-9">
			<div class="callout callout-info" id="printable">
				<div>
					<h3 class="text-center">Self Evaluation Result</h3>
					<hr>
					<table width="100%">
						<tr>
							<td width="50%">
								<p><b>Faculty: <span id="nameField"></span></b></p>
							</td>
							<td width="50%">
...
<script>
	...
	function load_class() {
		start_load()
		$.ajax({
			url: "ajax.php?action=get_faceval",
			method: 'POST',
				...
				if (resp) {
					...
					if (Object.keys(resp).length <= 0) {
						...
						Object.keys(resp).map(k => {
							$('#class-list').append('<a href="javascript:void(0)" data-json=\'' + JSON.stringify(resp[k]) + '\' data-id="' + resp[k].id + '" class="list-group-item list-group-item-action show-result">' + resp[k].faculty)
						})
	...
	function load_report($faculty_id, $fid) {
		if ($('#preloader2').length <= 0)
			start_load()
		$.ajax({
			url: 'ajax.php?action=get_faceval_report',
			method: "POST",
