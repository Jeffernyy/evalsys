...
if (isset($_GET['class_id'])) {
	$qry = $conn->query("SELECT * FROM class_l where class_id={$_GET['class_id']}")->fetch_array();
	foreach ($qry as $k => $v) {
		$$k = $v;
	}
}
...
<div class="container-fluid">
	<form action="" id="manage-class">
		...
		<div class="form-group">
			<label for="prog_code" class="control-label">Course Program</label>
			<input type="text" class="form-control form-control-sm" name="program" id="program"
				value="<?php echo isset($program) ? $program : '' ?>" required>
		</div>
		<div class="form-group">
			<label for="description" class="control-label">Year Level</label>
			<input type="text" class="form-control form-control-sm" name="year" id="year"
				value="<?php echo isset($year) ? $year : '' ?>" required>
		</div>
		<div class="form-group">
			<label for="section" class="control-label">Set</label>
