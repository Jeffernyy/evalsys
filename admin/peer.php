			...
			<div class="callout callout-info" id="printable">
				<div>
					...
					<table width="100%">
						...
						<tr>
							<td width="50%">
								<p><b>Group No.: <span id="subjectField"></span></b></p>
							</td>
							<td width="50%">
					...
					<p class=""><b>Total Peer Respondent: <span id="tse"></span></b></p>
				</div>
...
<script>
	$(document).ready(function () {
		$('#faculty_id').change(function () {
			if ($(this).val() > 0)
				window.history.pushState({}, null, './index.php?page=peer&fid=' + $(this).val());
			load_class()
		})
		if ($('#faculty_id').val() > 0)
			load_class()
	...
	function anchor_func() {
		$('.show-result').click(function () {
			...
			window.history.pushState({}, null, './index.php?page=peer&fid=' + vars.fid + '&gid=' + data.group_id);
			load_report(vars.fid, data.group_id);
	...
	function load_report($faculty_id, $group_id) {
		if ($('#preloader2').length <= 0)
			start_load()
		$.ajax({
			url: 'ajax.php?action=get_group_report',
			method: "POST",
			data: { faculty_id: $faculty_id, group_id: $group_id },
			error: function (err) {
				console.log(err)
				alert_toast("An Error Occured.", "error");
				end_load()
