				...
				<div class="row">
							...
							<div class="card-footer">
								<div class="d-flex justify-content-end w-100">
									<button class="btn btn-sm btn-primary btn-flat bg-gradient-primary mx-1"
										form="manage-criteria">Save</button>
									<button class="btn btn-sm btn-flat btn-secondary bg-gradient-secondary mx-1" form="manage-criteria"
					...
					<div class="col-md-8">
						<div class="callout callout-info">
							<?php
							$qry = $conn->query("SELECT * FROM criteria_list WHERE type=1 order by abs(order_by) asc ");
							if ($qry->num_rows > 0):
								?>
								<div class="d-flex justify-content-between w-100">
									<label for=""><b>Student Criteria List</b></label>
											...
											<li class="list-group-item text-left">
												<span class="btn-group dropright float-right">
													...
													<div class="dropdown-menu">
														...
														<a class="dropdown-item delete_criteria" href="javascript:void(0)"
															data-id="<?php echo $row['id'] ?>">Delete </a>
													</div>
												</span>
												<i class="fa fa-bars"></i>
												<?php echo ucwords($row['criteria']) ?>
					...
					<div class="col-md-8">
						<div class="callout callout-info">
							<?php
							$qry = $conn->query("SELECT * FROM criteria_list WHERE type=2 order by abs(order_by) asc ");
							if ($qry->num_rows > 0):
								?>
								<div class="d-flex justify-content-between w-100">
									<label for=""><b>Faculty Criteria List</b></label>
									<!--<button class="btn btn-sm btn-primary btn-flat bg-gradient-primary mx-1" form="order-criteria">Save Order</button>-->
								</div>
								<hr>
								<form action="" id="order-criteria">
					...
					<div class="col-md-8">
						<div class="callout callout-info">
							<?php
							$qry = $conn->query("SELECT * FROM criteria_list WHERE type=3 order by abs(order_by) asc ");
							if ($qry->num_rows > 0):
								?>
								<div class="d-flex justify-content-between w-100">
									<label for=""><b>Supervisor Criteria List</b></label>
									<!--<button class="btn btn-sm btn-primary btn-flat bg-gradient-primary mx-1" form="order-criteria">Save Order</button>-->
								</div>
								<hr>
								<form action="" id="order-criteria">
...
<script>
	$(document).ready(function () {
		...
		$('.edit_criteria').click(function () {
			...
			var criteria = <?php echo json_encode(array($criteria, $criteria2, $criteria1)) ?>;
			$('#manage-criteria').find("[name='id']").val(criteria[id].id)
			$('#manage-criteria').find("[name='criteria']").val(criteria[id].criteria)
			$('#manage-criteria').find("[name='type']").val(criteria[id].type).trigger('change')
