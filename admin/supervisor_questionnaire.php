					...
					<form action="" id="manage-question">
						...
						<div class="form-group">
							...
							<select name="criteria_id" id="criteria_id" class="custom-select custom-select-sm select2">
								<option value=""></option>
								<?php
								$criteria = $conn->query("SELECT * FROM criteria_list WHERE type=3 order by abs(order_by) asc ");
						...
						while ($crow = $criteria->fetch_assoc()):
							...
							<table class="table table-condensed">
								...
								<tbody class="tr-sortable">
									<?php
									$questions = $conn->query("SELECT * FROM visor_q where criteria_id = {$crow['id']} and academic_id = $id order by abs(order_by) asc ");
											...
											<td class="p-1 text-center" width="5px">
												<span class="btn-group dropright">
													...
													<div class="dropdown-menu">
														<a class="dropdown-item edit_visquestion" href="javascript:void(0)"
															data-id="<?php echo $row['id'] ?>">Edit</a>
														<div class="dropdown-divider"></div>
														<a class="dropdown-item delete_visquestion" href="javascript:void(0)"
...
<script>
	...
	$('.edit_visquestion').click(function () {
	...
	$('.delete_question').click(function () {
		_conf("Are you sure to delete this question?", "delete_visquestion", [$(this).attr('data-id')])
	})
	...
	$('#manage-question').submit(function (e) {
		...
		$.ajax({
			url: 'ajax.php?action=save_visquestion',
	...
	$('#order-question').submit(function (e) {
		...
		start_load()
		$.ajax({
			url: 'ajax.php?action=save_visquestion_order',
			data: new FormData($(this)[0]),
			cache: false,
			contentType: false,
			processData: false,
	...
	function delete_visquestion($id) {
		start_load()
		$.ajax({
			url: 'ajax.php?action=delete_visquestion',
			method: 'POST',
			data: { id: $id },
			success: function (resp) {
				if (resp == 1) {
