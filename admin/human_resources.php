      ...
      <div class="callout callout-info" id="printable">
        <div>
          ...
          <table width="100%">
            ...
            <tr>
              <td width="50%">
                <p><b>Evaluator: <span id="nameField"></span></b></p>
              </td>
              <td width="50%">
...
<script>
  $(document).ready(function () {
    $('#faculty_id').change(function () {
      if ($(this).val() > 0)
        window.history.pushState({}, null, './index.php?page=supervisor&fid=' + $(this).val());
      load_class()
    })
    if ($('#faculty_id').val() > 0)
      load_class()
  ...
  function load_class() {
    ...
    $.ajax({
      url: "ajax.php?action=get_human_resources",
      method: 'POST',
  ...
  function anchor_func() {
    $('.show-result').click(function () {
      ...
      window.history.pushState({}, null, './index.php?page=supervisor&fid=' + vars.fid);
      load_report(vars.fid, data.vid);
      $('#nameField').text(data.visor)
  ...
  function load_report($faculty_id, $vid) {
    if ($('#preloader2').length <= 0)
      start_load()
    $.ajax({
      url: 'ajax.php?action=get_visor_report',
      method: "POST",
      data: { faculty_id: $faculty_id, vid: $vid },
      error: function (err) {
        console.log(err)
        alert_toast("An Error Occured.", "error");
        end_load()
