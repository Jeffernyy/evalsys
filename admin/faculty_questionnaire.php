			...
			<div class="card card-info card-primary">
					...
					<form action="" id="manage-question">
						...
						<div class="form-group">
							...
							<select name="criteria_id" id="criteria_id" class="custom-select custom-select-sm select2">
								<option value=""></option>
								<?php
								$criteria = $conn->query("SELECT * FROM criteria_list WHERE type=2 order by abs(order_by) asc ");
				...
				<div class="card-footer">
					<div class="d-flex justify-content-end w-100">
						<button class="btn btn-sm btn-primary btn-flat bg-gradient-primary mx-1"
							form="manage-question">Save</button>
						<button class="btn btn-sm btn-flat btn-secondary bg-gradient-secondary mx-1" form="manage-question"
			...
			<div class="card card-outline card-info">
				<div class="card-header">
					<b>Evaluation Questionnaire for Academic:
						<?php echo $year . ' ' . (ordinal_suffix($semester)) ?>
					</b>
					<div class="card-tools">
						...
						<button class="btn btn-sm btn-flat btn-success bg-gradient-success mx-1" form="order-question">Save
							Order</button>
					</div>
				</div>
				<div class="card-body">
						...
						while ($crow = $criteria->fetch_assoc()):
							...
							<table class="table table-condensed">
								...
								<tbody class="tr-sortable">
									<?php
									$questions = $conn->query("SELECT * FROM faculty_que where criteria_id = {$crow['id']} and academic_id = $id order by abs(order_by) asc ");
											...
											<td class="p-1 text-center" width="5px">
												<span class="btn-group dropright">
													...
													<div class="dropdown-menu">
														<a class="dropdown-item edit_facquestion" href="javascript:void(0)"
															data-id="<?php echo $row['id'] ?>">Edit</a>
														<div class="dropdown-divider"></div>
														<a class="dropdown-item delete_facquestion" href="javascript:void(0)"
...
<script>
	...
	$('.edit_facquestion').click(function () {
	...
	$('.delete_question').click(function () {
		_conf("Are you sure to delete this question?", "delete_facquestion", [$(this).attr('data-id')])
	})
	...
	$('#manage-question').submit(function (e) {
		...
		$.ajax({
			url: 'ajax.php?action=save_facquestion',
	...
	$('#order-question').submit(function (e) {
		...
		start_load()
		$.ajax({
			url: 'ajax.php?action=save_facquestion_order',
			data: new FormData($(this)[0]),
			cache: false,
			contentType: false,
			processData: false,
	...
	function delete_facquestion($id) {
		start_load()
		$.ajax({
			url: 'ajax.php?action=delete_facquestion',
			method: 'POST',
			data: { id: $id },
			success: function (resp) {
				if (resp == 1) {
