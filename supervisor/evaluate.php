...
$curriculum = $conn->query("SELECT f.id as fid, concat(f.firstname,' ',f.lastname) as faculty FROM faculty_list f where f.id not in (SELECT faculty_id from visor_eval_list where academic_id ={$_SESSION['academic']['id']} and visor_id = {$_SESSION['login_id']} )");
?>
	...
	<div class="row">
		<div class="col-md-3">
			<div class="list-group">
				...
				?>
				<a class="list-group-item list-group-item-action <?php echo isset($faculty_id) && $faculty_id == $row['fid'] ? 'active' : '' ?>" href="./index.php?page=evaluate&fid=<?php echo $row['fid'] ?>&fid=<?php echo $row['fid'] ?>"><?php echo ucwords($row['faculty'])?></a>
			<?php endwhile; ?>
			</div>
		</div>	
		<div class="col-md-9">
			<div class="card card-outline card-info">
				...
				<div class="card-body">
					...
					<form id="manage-evaluation">
						<input type="hidden" name="visor_id" value="<?php echo $_SESSION['login_id'] ?>">
...
<script>
	...
	$('#manage-evaluation').submit(function(e){
		...
		start_load()
		$.ajax({
			url:'ajax.php?action=save_visor_eval',
			method:'POST',
			data:$(this).serialize(),
