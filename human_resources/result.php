...
<?php $supervisor_id = $_SESSION['login_id'] ?>
<?php 
...
<script>
	...
	function load_class(){
		...
		$.ajax({
			...
			method:'POST',
			data:{fid:<?php echo $supervisor_id ?>},
			error:function(err){
				console.log(err)
	...
	function anchor_func(){
		$('.show-result').click(function(){
			...
			load_report(<?php echo $supervisor_id ?>,data.sid,data.id);
	...
	function load_report($supervisor_id, $subject_id,$class_id){
		if($('#preloader2').length <= 0)
		start_load()
		$.ajax({
			url:'ajax.php?action=get_report',
			method:"POST",
			data:{supervisor_id: $supervisor_id,subject_id:$subject_id,class_id:$class_id},
			error:function(err){
				console.log(err)
				alert_toast("An Error Occured.","error");
				end_load()
